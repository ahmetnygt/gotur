...
exports.getTrips = async (req, res, next) => {
    try {
        // 🔑 modelleri tenant DB’den alıyoruz
        const { Trip, RouteStop, Stop, Route, Price, BusModel, Ticket } =
            req.models;
        ...
        if (!fromId || !toId || !date) {
            return res
                .status(400)
                .json({ message: "Eksik bilgi gönderildi." });
        }
        ...
        if (
            !fromPlaceStopIds?.length ||
            !toPlaceStopIds?.length
        ) {
            return res.json([]);
        }
        ...
        if (!routeStops.length) return res.json([]);
        ...
        const matchingRouteIds = Array.from(
            routesWithPlaces.values()
        )
            .filter(
                ({ placeIds }) =>
                    placeIds.has(fromPlaceKey) && placeIds.has(toPlaceKey)
            )
            .map(({ routeId }) => routeId);

        if (!matchingRouteIds.length) return res.json([]);
        ...
        const routeStopsOfTrips = await RouteStop.findAll({
            where: { routeId: { [Op.in]: [...new Set(routesOfTrips.map((r) => r.id))] } },
            raw: true,
        });
        ...
        for (let i = 0; i < trips.length; i++) {
            ...
            const fromRouteStopOrder = _routeStops.find(
                (rs) => rs.stopId == stops.find((s) => s.placeId == fromId).id
            ).order;
            const toRouteStopOrder = _routeStops.find(
            ...
            if (fromRouteStopOrder !== _routeStops.length - 1) {
                for (let j = 0; j < _routeStops.length; j++) {
                    ...
                    if (rs.order == fromRouteStopOrder) break;
                }

                for (let j = 0; j < _routeStops.length; j++) {
                    const rs = _routeStops[j];
                    if (
                        rs.order > fromRouteStopOrder &&
                        rs.order <= toRouteStopOrder
                    ) {
